        ...
        <MenuList className="flex flex-col gap-1">
          {Languages.map((lang) => (
            <MenuItem
              ...
              className={` ${lang.value === language && '!bg-[--bgSoft]'} flex items-center justify-between`}
            >
              <span>{lang.label}</span>
              <FiCheck
                className={` ${lang.value === language ? `!opacity-100` : `!opacity-0`} transition-opacity ease-in-out`}
              />
            </MenuItem>
          ))}
        </MenuList>
